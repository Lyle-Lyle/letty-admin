...
import { Dashboard, Dept, Login, Menu, ResultData, User, Register } from '@/types/api';
export default {
    // 登录
    login(params: Login.params) {
        return request.post<string>('/user/login', params, { showLoading: false });
    },
    // 注册
    register(params: Register) {
        return request.post<string>('/user/register', params, { showLoading: false });
    },
    ...
    // 获取当前账号下的所有用户
    getAllUserList() {
        return request.get<User.UserItem[]>('/user/all/list');
    },
    // 创建部门
